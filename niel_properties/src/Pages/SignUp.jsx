...
const SignUp = () => {
  ...
  return (
    // FIX 1: 'min-h-screen' allows mobile to scroll. 'lg:h-screen' locks desktop height.
    <div className="min-h-screen lg:h-screen bg-white font-sans flex flex-col lg:flex-row items-center justify-center p-0 lg:p-6 overflow-x-hidden lg:overflow-hidden relative">
      ...
      {/* Back to Home Link */}
      {/* Position adjusted for mobile (top-4 right-4) and text color changes to white on mobile to be visible over the image */}
      ...
      <div className="w-full max-w-[1400px] h-full grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-24">
        ...
        {/* --- LEFT SIDE (Image) --- */}
        {/* Mobile: Height fixed (h-64), rounded bottom. Desktop: Height full, rounded all corners. Order-1 puts it at top. */}
        <div className="relative h-64 sm:h-80 lg:h-full w-full rounded-b-[3rem] lg:rounded-[2rem] overflow-hidden order-1">
          ...
          {/* --- Bottom Content Wrapper with Overlapping Image --- */}
          {/* Hidden on mobile (hidden lg:block) to avoid clutter */}
        ...
        {/* --- RIGHT SIDE (Form) --- */}
        {/* Mobile: Order-2 (below image), with padding. Desktop: Centered vertically. */}
